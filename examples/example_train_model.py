...
# n_pos = label_stats.filter("churn = 1").select("cnt").first()[0] if label_stats.filter("churn = 1").count() else 0
# n_neg = label_stats.filter("churn = 0").select("cnt").first()[0] if label_stats.filter("churn = 0").count() else 0
...
#     train_df
#     .withColumn("weight",
#                 F.when(F.col("churn") == 1, F.lit(w_pos))
#                  .otherwise(F.lit(w_neg)))
# )

# train_df = train_df.withColumn(
#     "page_bucket",
#     F.when(F.col("page_bucket").isNull() | (F.col("page_bucket") == ""), "Other").otherwise(F.col("page_bucket"))
# )
...
# train_df = train_df.withColumn(
#     "database_name",
#     F.when(F.col("database_name").isNull() | (F.col("database_name") == ""), "unknown_db").otherwise(F.col("database_name"))
# )
...
# num_cols = [
#     "active_days", "avg_norm", "std_norm", "max_norm",
#     "views_last1", "views_last3", "views_last7", "is_mobile_int"
# ]
...
# roc_eval = BinaryClassificationEvaluator(labelCol="churn", rawPredictionCol="rawPrediction", metricName="areaUnderROC")
...
# pr_eval = BinaryClassificationEvaluator(labelCol="churn", rawPredictionCol="rawPrediction", metricName="areaUnderPR")
...
# pred_test.select("churn", (F.col("probability")[1] > 0.5).cast("int").alias("pred")).groupBy("churn","pred").count().show()
